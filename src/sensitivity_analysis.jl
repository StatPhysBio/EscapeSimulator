export Bins, count!, hellinger, observed_discrepancy

struct Bin # 1-D membership function
    low::Float64
    hi::Float64
end
...
struct Bins
    Z::Array{Bin} # lenght(z) = n List of membership functions
...
function Bins(data; min = -Inf, max = +Inf) # censoring cutoffs of continuous measurement and number of measurements outside cutoff
    ...
    Z = [Bin(boundaries[ii], boundaries[ii+1]) for ii in 1:length(boundaries)-1]
    if  length(datapoints) == 0
        q = cat(lows, [0], highs; dims=1)
    else
...
function count!(x,b::Bins)
    ind = findfirst(z -> b.Z[z](x), eachindex(b.Z)) # ambiguity in "<=" resolved by findfirst
    b.count[ind] += 1
end
...
function observed_discrepancy(reference_data, simulated_data; min = 0.001, max = 56)
    bins = Bins(reference_data; min, max)
...
# These functions only apply to the simple model log X rebounds
